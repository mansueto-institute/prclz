...
if __name__ == "__main__":
	...
	parser = argparse.ArgumentParser("Split master buildings files by GADM to facilitate downstream parallelization by GADM")
	parser.add_argument("--bldg_file", required=True, help="Path to master geojson file containing all building polygons")
	parser.add_argument("--gadm_path", required=True, help="Path to GADM file")
	parser.add_argument("--output_dir", required=True, help="Output directory for gadm-specific building files")
	parser.add_argument("--level", default='WARNING')
	args = parser.parse_args()

	basicConfig(level=args.level.upper())
	del args.level

	split_bldgs_by_gadm(**vars(args))
